...
export default function LeadMap() {
    ...
    // isAdmin is a function, get the boolean value
    ...
    async function loadLeads() {
        ...
        try {
            ...
            console.log('[LeadMap] Loaded leads:', data.length, 'isAdmin:', userIsAdmin);
            setLeads(data);
        } catch (error) {
            ...
            <div className="bg-white border-b border-slate-200 px-6 py-4 flex justify-between items-center shadow-sm z-10">
                <div>
                    <h1 className="text-xl font-bold text-slate-800 flex items-center gap-2">
                        ...
                        Territory Map {userIsAdmin && <span className="text-xs bg-brand-100 text-brand-700 px-2 py-0.5 rounded-full">Admin View</span>}
                    </h1>
                    <p className="text-sm text-slate-500">
                        {loading ? 'Loading locations...' : `${leads.length} Locations${userIsAdmin ? ' (All)' : ' (Assigned to you)'}`}
                    </p>
                </div>
